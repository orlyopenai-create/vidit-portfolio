...
function StatCountUp({ value, label, suffix, decimals }: { value: number; label: string; suffix: string; decimals: number }) {
  ...
  return (
    <div ref={ref} className="flex flex-col items-center gap-1.5">
      <span className="font-mono text-[clamp(2.5rem,5vw,4.5rem)] text-[#C4832A] leading-none tracking-[-0.02em]">
        {count.toFixed(decimals)}{suffix}
      </span>
      <span className="font-body text-[0.7rem] font-medium tracking-[0.12em] uppercase text-[#F2EAE0]/45 text-center">
        {label}
      </span>
    </div>
  )
...
export function FundAnimations() {
  return (
    <div>
      ...
      <div className="mb-6">
        <ClipReveal>
          <h2 className="font-display text-[clamp(2.5rem,5vw,5rem)] font-normal leading-[1.05] tracking-[-0.01em] text-[#F2EAE0]">
            The Barbershop Fund
          </h2>
        </ClipReveal>
      </div>
      ...
      <m.p
        ...
        className="font-body text-[1rem] text-[#F2EAE0]/60 leading-relaxed max-w-2xl mb-12"
      >
      ...
      <m.div
        ...
        className="grid grid-cols-3 gap-8 mb-14 border-t border-[#F2EAE0]/8 pt-10"
      >
        {fundStats.map((stat) => (
      ...
      <m.div
        ...
        className="mb-14 border-t border-[#F2EAE0]/8 pt-10"
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {highlights.map((h, i) => (
            <m.div
              ...
              className="rounded-xl bg-[#161210] p-5"
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                <span className="font-display text-[1.05rem] text-[#F2EAE0]">{h.name}</span>
                <span className="font-mono text-[0.72rem] text-[#C4832A] shrink-0">{h.multiple}</span>
              </div>
              <p className="font-mono text-[0.65rem] text-[#C4832A]/55 uppercase tracking-widest mb-2">{h.tag}</p>
              <p className="font-body text-[0.85rem] text-[#F2EAE0]/50 leading-snug">{h.line}</p>
            </m.div>
          ))}
        </div>
      </m.div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            <p className="font-mono text-[0.65rem] uppercase tracking-widest text-[#C4832A]/60 mb-4">
              In Founders
            </p>
            <div className="space-y-2.5">
              {founderTraits.map((trait) => (
                <div key={trait} className="flex items-center gap-3">
                  <span className="w-1 h-1 rounded-full bg-[#C4832A]/50 shrink-0" />
                  <p className="font-body text-[0.9rem] text-[#F2EAE0]/60 leading-snug">{trait}</p>
                </div>
              ))}
            </div>
          </div>
